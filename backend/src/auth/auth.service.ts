...
export class AuthService {
  ...
  private static readonly DEFAULT_PROFILE_IMAGE =
    "https://static.wixstatic.com/media/5fdc83_7f6f3eb17d5d4e1584497cffd749463c~mv2.jpg/v1/fill/w_970,h_500,al_c,q_85,enc_avif,quality_auto/The_Snoopy_Show_970x500.jpg";

  private static readonly GOOGLE_CLIENT_IDS = [
  ...
  constructor(
    ...
    this.oauthClient = new OAuth2Client(AuthService.GOOGLE_CLIENT_IDS[0]);
  ...
  async registerUser(email: string, password: string, username: string) {
    ...
    const newUser = await this.prisma.user.create({
      data: {
        ...
        username,
        profileImage: AuthService.DEFAULT_PROFILE_IMAGE,
      },
    });
  ...
  async verifyUserEmail(token: string) {
    try {
      ...
      return this.renderVerifiedPage();
    } catch (error) {
  ...
  async loginUser(email: string, password: string) {
    ...
    return jwt.sign({ id: user.id }, this.jwtSecret, { expiresIn: "1h" });
  }

  async loginWithGoogle(googleToken: string) {
    const ticket = await this.oauthClient.verifyIdToken({
      idToken: googleToken,
      audience: AuthService.GOOGLE_CLIENT_IDS,
    });
  ...
  private renderVerifiedPage(): string {
    return `
		<!DOCTYPE html>
		<html>
		<head>
