...
export default function Profile() {
	...
	const openFriendPreview = async (friendUser: any, friendshipId: string) => {
	...
	const handleRemoveFriend = async () => {
        ...
        try {
            const response = await fetch(`${apiUrl}/friends/${selectedFriendshipId}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
            if (response.ok) {
                setFriendsList((prev) => prev.filter((friend) => friend.id !== selectedFriendshipId));
				...
				{friendsList.length === 0 ? (
					...
					<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.friendsScroll}>
						{friendsList.map((friend) => {
							...
							return (
								<TouchableOpacity key={friend.id} onPress={() => openFriendPreview(friendUser, friend.id)} activeOpacity={0.7}>
				...
				<View style={styles.modalOverlay}>
					<View style={styles.previewModalContent}>
                        ...
                        <TouchableOpacity style={styles.removeFriendButton} onPress={handleRemoveFriend}>
                            <Text style={styles.removeFriendButtonText}>Remove from my friend list</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
